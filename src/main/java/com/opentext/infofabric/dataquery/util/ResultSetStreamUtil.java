...
public class ResultSetStreamUtil {
    ...
    public int stream(ResultSet rs, String jobID, String streamName, String typeName, String tenant) throws Exception {
        int rowCount = 0;
//        ProcessorTask task = Registrar.getTaskForNewProcess(jobID, streamName);
        ProcessorTask task = registrarClient.newProcessorTask(tenant, streamName);
//        ProcessorTask task = null;
    ...
    private void sendToStream(final byte[] mappedData, final String key, ProcessorTask processorTask, String streamName) {
        try {
            Payload payload = new Payload(mappedData, key, MediaType.JSON, PayloadCommand.ASSERT);
            streamProducer.sendAsync(processorTask, streamName, payload);
    ...
    private void endProcessing(final ProcessorTask processorTask, final Status status, final String msg) {
        processorTask.addEvent(status, msg);
        processorTask.endProcessing(status);
    }


    public JSONArray getSyncResponse(ResultSet rs) throws Exception {
        JSONArray jsonArray = new JSONArray();
