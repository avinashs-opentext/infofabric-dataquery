...
import com.liaison.dataquery.graphql.dataloaders.hbase.model.Select;
import com.liaison.dataquery.graphql.dataloaders.hbase.model.Where;
...
public class HBaseQuery {
    ...
    public Map<String, Object> getByRowKey(RawQuery rq) throws IOException {
        ...
        Table table = null;
        try {
            table = dbc.getTable(TableName.valueOf(String.format(tablePath, config.getMaprAdminClientConfig().get(VOLUME_BASE_PATH_PROPERTY_NAME), dataCenterEnv, tenant, model) + "/" + rq.getTable()));
    ...
    public ResultObject gqlByRowKey(String rowKey, Map<String, String> selects, String tableName) throws IOException {
        ...
        try {
            table = dbc.getTable(TableName.valueOf(String.format(tablePath, config.getMaprAdminClientConfig().get(VOLUME_BASE_PATH_PROPERTY_NAME), dataCenterEnv, tenant, model) + "/" + tableName));
            Result[] results = table.get(gets);
    ...
    public List<Map<String, Object>> scan(RawQuery rq) throws IOException {
        ...
        Table table = dbc.getTable(TableName.valueOf(String.format(tablePath, config.getMaprAdminClientConfig().get(VOLUME_BASE_PATH_PROPERTY_NAME), dataCenterEnv, tenant, model) + "/" + rq.getTable()));
    ...
    private SingleColumnValueFilter getColumnValueFilter(com.liaison.dataquery.graphql.query.Filter f) {
        SingleColumnValueFilter singleColumnValueFilterA = null;
    ...
    public List<ResultObject> gqlScan(Query rq) throws IOException {
        ...
        Table table = dbc.getTable(TableName.valueOf(String.format(tablePath, config.getMaprAdminClientConfig().get(VOLUME_BASE_PATH_PROPERTY_NAME), dataCenterEnv, tenant, model) + "/" + rq.getCollection().getName()));
         ...
         * Set the filters
         */
        for (com.liaison.dataquery.graphql.query.Filter f : rq.getFilterSet().getFilters()) {
            if (f.isID()) {
